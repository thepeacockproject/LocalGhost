...
app.get('/dynamic_resources_pc_release_rpkg', (req, res) => {
    md5File(path.join('menudata', 'h2', 'dynamic_resources_pc_release_rpkg.rpkg')).then(hash => {
        res.sendFile(path.join('menudata', 'h2', 'dynamic_resources_pc_release_rpkg.rpkg'), {
            root: '.',
            headers: {
...
for (const folder of [
    ...
    app.use(`/${folder}/`, express.static(path.join('menudata', 'h2', folder), {
        index: false,
        redirect: false,
        dotfiles: 'ignore',
    }));
